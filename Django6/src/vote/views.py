...
def index(request):
    ...
    # : Question 모델 클래스에 저장된 모든 객체 추출
    list = Question.objects.all()
    #render(request, html 파일경로, 템플릿에 전달할 데이터-사전형
    return render(request, "vote/templates/index.html",
                  {'question_list' : list } )

def detail(request, question_id): 
    #get_object_or_404 : 모델클래스에 id값으로 객체 1개를 반환 함수
...
def vote(request,question_id):
    ...
    if request.method == "POST":
        ...
        #튜플을 만들때 요소 개수가 한개면 사칙연산에 사용하는 우선순위 괄호로
        #판단함. 때문에 튜플 요소 개수가 한개일 경우 끝에 쉼표를 입력
        return HttpResponseRedirect( 
            reverse('vote:result', args=( question_id, ) )   )
...
def registerQ(request):
    if request.method == "GET":
        ...
        #폼객채.is_valid() : 해당 폼에 입력값들이 에러가 없는지 확인
        #True, False 값 반환. 폼 객체 사용시 반드시 사용해야되는 함수
        if form.is_valid(): 
            #폼객체.save() : 해당 폼에 입력값들로 모델클래스 객체를
            #데이터베이스에 저장후 반환
            ...
            #request.user.get_username() : 로그인된 회원의 
            #                              username을 반환하는 함수
            ...
            #템플릿으로 form 전달하면 사용자가 이전에 작성한 내용이 들어있는
            #상태로 전달함
            return render(request, "vote/templates/registerQ.html",
...
def deleteC(request, choice_id):
    ...
    if request.user.get_username() != obj.question.author.username:
        return render(request,"vote/templates/error.html", {'error':"잘못된 접근 입니다.",'returnURL' : reverse('vote:detail', args=(obj.question.id,) ) } )
    
    
    id = obj.question.id #choice 객체 삭제전에 Question 객체의 id값을 저장
    obj.delete()
    ...
    #3. detail.html 파일(vote/templates/detail.html) 수정
    #각 답변 항목별로 '삭제'링크 만들기
...
def registerC(request, question_id):
    ...
    if request.user.get_username() != obj.author.username:
        return render(request,"vote/templates/error.html", {'error':"본인이 작성한 글이 아닙니다.",'returnURL' : reverse('vote:detail',args=(question_id,) ) })
    
    if request.method == "GET":
        #폼 객체 생성
        form = ChoiceForm()
        #render함수로 HTML파일 로드 + 탬플릿에 폼객체 전달
        return render(request,"vote/templates/registerC.html",
                      {'form':form, 'name' : obj.question_text})
...
def updateQ(request, question_id):
    ...
    if request.method == "GET":
        ...
        #모델폼의 생성자에 instance 매개변수는 이미 생성된 모델클래스의 객체를
        #담을 수 있음
        form = QuestionForm(instance = obj)
        return render(request,"vote/templates/updateQ.html",
                      {'form':form})
    elif request.method == "POST":
        #이미 생성된 Question 객체에 내용을 클라이언트가 작성한 내용으로
        #덮어씌움
